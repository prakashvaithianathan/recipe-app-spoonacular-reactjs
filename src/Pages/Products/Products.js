...
const linkHover = (index) => {
  document.querySelector(`#learn${index}`).style.color ='red';
};

const linkOut = (index) => {
  document.querySelector(`#learn${index}`).style.color ='inherit'
}
...
const Products = () => {
  ...
  if (!products) {
    return <h1 className={style.loading}>No Products Found</h1>;
  }
      ...
      {product[0].searchResults[0].results.map((data, index) => {
        return (
          ...
          <Link
            ...
            onMouseOverCapture={()=>linkHover(index)}
            onMouseOutCapture={()=>linkOut(index)}
            key={index}
          >
            <Card
